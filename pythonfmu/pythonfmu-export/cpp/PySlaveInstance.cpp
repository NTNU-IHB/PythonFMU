...
{
    bool status;
    PyState::run([this, &status, &currentTime, &stepSize] {
...
{
    PyState::run([this, &vr, &nvr, &values] {
...
{
    PyState::run([this, &state, &bytes, &size] {
        auto pyState = reinterpret_cast<PyObject*>(state);
        PyObject* pyStateBytes = PyObject_CallMethod(pClass_, "_fmu_state_to_bytes", "(O)", pyState);
        if (pyStateBytes == nullptr) {
...
{
    PyState::run([this, &bytes, &size, &state] {
